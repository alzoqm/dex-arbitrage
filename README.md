...
- **Router**
  - V2-only N-hop cycle은 constant-product 합성식으로 최적 투입량을 먼저 산출
  - V2 local exact math
  - V3 quoter 기반 exact quote (quoter 없으면 보수적 fallback)
  - Curve `get_dy` exact quote
  - Balancer weighted fallback quote
  - V2 parallel pool은 marginal water-filling으로 split allocation 산출
  - V3/Curve/Balancer 또는 혼합 경로는 기존 exact quote 기반 탐색으로 fallback
- **Execution**
  - route calldata 생성
